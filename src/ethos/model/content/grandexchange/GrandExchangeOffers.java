...
import ethos.model.content.grandexchange.*;
import ethos.model.content.grandexchange.GrandExchangeOffer.OfferType;
...
public class GrandExchangeOffers {
	...
	public static void add(GrandExchangeOffer offer) {
		...
		CycleEventHandler.getSingleton().addEvent(offer,new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
			for(GrandExchangeOffer o : OFFERS) {
				if(o == null || o == offer)
					continue;
				Player p = PlayerHandler.getPlayer(o.getOwner());
				int item1 = offer.getId();
				if(p.getItems().isNoted(item1)) {
					item1 = Item.getUnNoted(item1);
				}

				int item2 = o.getId();
				if(p.getItems().isNoted(item2)) {
					...
					if(o.getType() == OfferType.SELLING && offer.getType() == OfferType.BUYING) {
						if(o.getPricePerItem() > offer.getPricePerItem() && o.getAmountFinished() < o.getAmount()) {
							...
							if(o.getFailAttempts() >= 3) {
								if(p != null) {
									p.sendMessage("<img=10> <col=996633>Perhaps you should try lowering the price on your "+ItemDefinition.forId(item2).getName()+" offer");
									p.sendMessage("<col=996633>in the Grand Exchange. People are currently paying less for that item.");
									o.setFailAttempts(0);
								}
							}
							continue;
	...
	public static int getGoodOffer(int item, OfferType type) {
		...
		for(GrandExchangeOffer o : OFFERS) {
			if(o == null || o.getType() == type || o.getAmountFinished() >= o.getAmount())
				continue;
			if(ItemDefinition.isNoted(item)) {
				item = Item.getUnNoted(item);
			}
			int offerItem = o.getId();
			if(ItemDefinition.isNoted(offerItem)) {
